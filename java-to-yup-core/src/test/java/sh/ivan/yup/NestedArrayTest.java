...
class NestedArrayTest extends JavaToYupConverterTest {
    @Test
                        ...
                        new ArraySchema(
                                new ArraySchema(
                                        new StringSchema(Set.of(
                                                ...
                                                new SizeAttribute(0, 12))),
                                        Set.of(
                                                new DefinedAttribute(),
                                                new SizeAttribute(2, Integer.MAX_VALUE),
                                                new SizeAttribute(0, 20))),
                                Set.of(new DefinedAttribute(), new SizeAttribute(1, Integer.MAX_VALUE))));
        objectSchemaAssert
                .extracting(ObjectSchema::asYupSchema)
                .isEqualTo(
                        "object({ names: array().of(string().defined().min(3).max(24)).defined().min(1), listOfLists: array().of(array().of(string().defined().min(5).max(12)).defined().min(2).max(20)).defined().min(1), })");
    }
